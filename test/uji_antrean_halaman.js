...
(async () => {
     ...
     ================================================================ */
  {
        ...
        createClient: () => ({
          rpc: async (nama, arg) => {
            ...
            return { data: {
              ...
              klinik: 'Laboratorium Medis Utama', judul: 'Antrean Pasien',
    ...
    cek('nama klinik tampil',
        (await page.locator('#namaKlinik').textContent()).includes('Utama'));

    /* ---------------------------------------------------------------
       INI PEMERIKSAAN YANG PALING PENTING DI BERKAS INI.

       Layar dipasang di ruang tunggu dan dilihat semua orang. Keputusan
       Fadzkal: nomor saja. Yang diperiksa bukan niat kodenya melainkan
