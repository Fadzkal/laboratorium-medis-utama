# RME Laboratorium Medis Utama — Panduan Pemasangan
...
- Akun **GitHub** — tempat kode ini di-push (repo `Fadzkal/RME-Klinik-Utama`)
...
```sql
update pegawai
set peran = 'master', nama = 'dr. Fadzkal'
...
Kode ada di repo GitHub `Fadzkal/RME-Klinik-Utama`. Cloudflare Pages
...
3. Pilih repo `RME-Klinik-Utama`, klik **Begin setup**.
4. Isi pengaturan build:
   - **Project name**: mis. `rme-lab-utama` (menentukan alamat `rme-lab-utama.pages.dev`)
...
Cloudflare memberi alamat `https://rme-lab-utama.pages.dev` (atau nama project Anda).
...
Bentuknya ditetapkan Yayasan Kesehatan Utama dan disusun oleh sistem:
...
- **Garis hitam di bawah kop** — bawaannya mati; kop Laboratorium Medis Utama sudah punya garis
  hijau sendiri.
...
Aplikasi ini sengaja dirancang supaya mudah disatukan dengan Pantau Laboratorium Medis Utama nanti:
